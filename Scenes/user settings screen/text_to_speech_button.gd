...
func _ready() -> void:
	# Connect the toggle button's toggled signal to the handler function
	check_button.toggled.connect(_on_check_button_toggled)

# Called when the CheckButton is toggled
func _on_check_button_toggled(toggled_on: bool) -> void:
	# Update the label based on the new toggle state
	set_label_text(toggled_on)

# Updates the label text based on the button's toggle state
